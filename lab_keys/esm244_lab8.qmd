...
library(ggfortify) # For PCA biplot (autoplot)
...
# pivot data for plotting, get all dimensions into a column
hdi_data_long <- hdi_data_raw %>%
  pivot_longer(names_to = 'name', values_to = 'value', where(is.numeric)) # where() tells it to select all columns where the condition is true

# plot
ggplot(hdi_data_long, aes(x = value)) +
  geom_histogram() +
  facet_wrap(~ name, scales = 'free_x') # individual plot for each different variable
# check if any data is highly skewed (could log transform those variables if needed to get them into a normal distribution)
```
...
hdi_data <- hdi_data_raw %>%
  drop_na() %>%
  mutate(hdicode = factor(hdicode, levels = c('Low', 'Medium', 'High', 'Very High'))) %>% # assigning development index levels to be in the order we want them for plotting
  ### why are we log transforming?  PCA does not require normality.  
  ### But see HDI description - they use log gnipc for their scoring.
  mutate(gnipc_2021 = log(gnipc_2021)) # log reduces the range of the data to be more easily worked with

# running pca using prcomp() function
  # note: prcomp() can't handle missing values
hdi_pca <- hdi_data %>% 
  select(where(is.numeric)) %>% # so that it's ready for pca (can only take numeric info)
  # select(-iso3, -country, -hdicode)) %>%
  # select(ends_with('2021'), lat) %>%
  prcomp(scale = TRUE) # rescale all our data to have mean of 0, SD of 1, so that they are comparable
str(hdi_pca)  
```
...
List of 5
 ...
 $ rotation: num [1:9, 1:9] 0.368 0.35 0.35 0.354 -0.363 ... # how much of each variable is being used to construct PC (matrix)
...
# can see how variables convert to PCs and vice versa
hdi_pca$rotation
```
...
```{r}
# getting a sense of what our data looks like
# first: gross national income per capita vs life expectancy
ggplot() +
  geom_point(data = hdi_data, aes(x = gnipc_2021, y = le_2021, color = hdicode)) +
  theme_minimal()

# second: expected years of schooling by mean years of school
ggplot() +
...
```{r}
# hard way = by hand and plot via ggplot

# make data frame. start with our rotation matrix from above
loadings_df <- data.frame(hdi_pca$rotation * 8) %>% ### 8x multiplier is arbitrary, to extend the length of the arrows
  mutate(axis = row.names(.)) # adds row names as a column

new_pts_df <- data.frame(hdi_pca$x) # x value is all the original observations, but translated to our PCA coordinate system
  # the x will be the points that we add to our PCA plot
  # resulting df gives a variable column for each PC

# plot
ggplot() +
  geom_point(data = new_pts_df, aes(x = PC1, y = PC2), # axes are PC1 and PC2
             color = 'blue') +
  geom_segment(data = loadings_df,
               x = 0, y = 0, # start segments at 0,0
               aes(xend = PC1, yend = PC2, group = axis), # drawing line segments till the end at the respective PC1 and PC2 values
               arrow = arrow(length = unit(.25, 'cm'))) + # put arrow on the end of our segments
  geom_text(data = loadings_df,
            aes(x = PC1, y = PC2, label = axis), vjust = 0, nudge_y = .01) + # add labels of variable names to each arrow
  theme_minimal()

## ggrepel can be used to help avoid overlap of labels
```

and by autoplot

``` {r}
# extra add ins from ggfortify package
autoplot(hdi_pca, # give pca results
     	data = hdi_data, # give it our raw data
     	loadings = TRUE, # add arrows
     	colour = 'hdicode', # color based on the hdi code levels
     	loadings.label = TRUE, # label our arrows
...
```{r}
# create some vectors
sd_vec <- hdi_pca$sdev # gives as a vector
var_vec <- sd_vec^2 ### standard deviation is sqrt of variance!
pc_names <- colnames(hdi_pca$rotation)

# make data frame
pct_expl_df <- data.frame(v = var_vec,
                          pct_v = var_vec / sum(var_vec), # var_vec is the variance explained by dif axes. this is variace of each component divided by total summed variance
                          pc = pc_names) %>% # from our rotation matrix
  mutate(pct_lbl = paste0(round(pct_v*100, 1), '%')) # turning percent into a nice looking text tabel

# plot our data frame
ggplot(pct_expl_df, aes(x = pc, y = v)) +
  geom_col() +
  geom_text(aes(label = pct_lbl), vjust = 0, nudge_y = .01) +
  labs(x = 'Principal component', y = 'Variance explained')
# can see that the first PC explains a ton of the variance! the others are much less relevant
```
